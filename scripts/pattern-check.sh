...
# Check for console.log as the only error handling in catch blocks
check_console_error_handling() {
    local file="$1"

    if is_check_disabled "CONSOLE_ERROR_HANDLING"; then
        return
    fi

    # Find catch blocks where console.log/warn is the ONLY statement
    awk '
    /catch\s*\([^)]*\)\s*\{/ {
        catch_line = NR
        in_catch = 1
    ...
    in_catch {
        ...
        body = body "\n" $0
        if (brace_count <= 0) {
            # Check if body ONLY contains console.log/warn/error calls
            cleaned = body
            gsub(/[ \t\n]/, "", cleaned)
            gsub(/console\.(log|warn|error)\([^)]*\);?/, "", cleaned)
            gsub(/\/\/[^\n]*/, "", cleaned)
            if (length(cleaned) == 0 && body ~ /console\.(log|warn|error)/) {
                print catch_line
            }
            in_catch = 0
        }
    ...
    ' "$file" 2>/dev/null | while read -r line_num; do
        add_violation "high" "CONSOLE_ERROR_HANDLING" "$file" "$line_num" "console.log as only error handling in catch block"
    done
}

# Check for async functions that never use await
check_unused_async() {
    local file="$1"

    if is_check_disabled "UNUSED_ASYNC"; then
        return
    fi

    # Find async function declarations and check for await usage
    awk '
    /async\s+(function\s+\w+|(\w+|\([^)]*\))\s*=>)\s*\{/ {
        func_line = NR
        in_func = 1
        brace_count = 1
        has_await = 0
    ...
    in_func {
        if (/await\s/) has_await = 1
        brace_count += gsub(/{/, "{") - gsub(/}/, "}")
        if (brace_count <= 0) {
            if (!has_await) print func_line
            in_func = 0
        }
    }
    ' "$file" 2>/dev/null | while read -r line_num; do
        add_violation "medium" "UNUSED_ASYNC" "$file" "$line_num" "async function without await"
    done
}
...
check_eslint_disable() {
    ...
    if is_check_disabled "ESLINT_DISABLE"; then
        return
    fi
...
check_any_type() {
    ...
    if is_check_disabled "ANY_TYPE"; then
        return
    fi
    ...
    while IFS=: read -r line_num line_content; do
        add_violation "high" "ANY_TYPE" "$file" "$line_num" "TypeScript 'any' type — use specific type instead"
    done < <(grep -n -E ':\s*any\b|<any>|as\s+any\b' "$file" 2>/dev/null || true)
}
...
main() {
    ...
    while IFS= read -r file; do
        ...
        check_console_error_handling "$file"
        check_unused_async "$file"
        check_eslint_disable "$file"
        check_any_type "$file"
    done <<< "$files"
...
record_metrics() {
    ...
    # Also check agent-kit location
    if [[ ! -f "$metrics_script" ]]; then
        metrics_script="${SETUP_REPO:-$HOME/Projects/agent-kit}/scripts/pattern-metrics.sh"
    fi
    ...
    local console_error_handling=0 unused_async=0 eslint_disable=0 any_type=0
    ...
    while IFS='|' read -r severity pattern file line message; do
        ...
        case "$pattern" in
            ...
            CONSOLE_ERROR_HANDLING) ((console_error_handling++)) || true ;;
            UNUSED_ASYNC)           ((unused_async++)) || true ;;
            ESLINT_DISABLE)         ((eslint_disable++)) || true ;;
            ANY_TYPE)               ((any_type++)) || true ;;
        esac
    done < "$VIOLATIONS_FILE"
    ...
    violations_json=$(cat << EOF
{
    ...
    "CONSOLE_ERROR_HANDLING": $console_error_handling,
